...
void setupSens() {
	// why these rates?
	SerialGPS.begin(115200, SERIAL_8N1, rxInGPS, txOutGPS);
	SerialTrans.begin(57600, SERIAL_8N1, rxRadio, txRadio);
	//serial test
	SerialTrans.println("HI");
	while (!Serial) {
		if (millis() > 500) {
			break;
		}
	...
	if (!mpu.begin(0x68)) {
		SerialTrans.println("Failed to find MPU6050 chip");
		while (1) {
			delay(10);
		}
	}
	if (!dps.begin_I2C(0x77)) {  // Can pass in I2C address here
		SerialTrans.println("Failed to find DPS");
		while (1) yield();
	}
